...
# the image build to sign the /usr IPE policy and verity root hash.
...
if [[ -n "${EPHEMERAL_CERT_DIR}" ]]; then
    # Reuse the cert that already signed the /usr IPE policy and detached
    ...
    info "Reusing shared per-build ephemeral cert (also signs the IPE assets)"
else
