...
# uki_install.sh to sign the embedded IPE policy and /usr verity root hash.
...
if [[ -n "${EPHEMERAL_CERT_DIR}" ]]; then
    # Reuse the cert that already signed the embedded IPE policy and /usr
    # verity root hash. The same cert must sign the UKI and be enrolled in the
    # VM db so the kernel can verify all signatures against .platform.
