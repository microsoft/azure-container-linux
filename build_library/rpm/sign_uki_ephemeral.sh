...
# uki_install.sh to sign the embedded IPE policy and /usr verity roothash.
...
if [[ -n "${EPHEMERAL_CERT_DIR}" ]]; then
    # Reuse the cert that already signed the embedded IPE policy and optional
    # /usr verity roothash. The same cert must sign the UKI and be enrolled in
    # the VM db so the kernel can verify both signatures against .platform.
    ...
    info "Reusing shared per-build ephemeral cert (also signs the embedded IPE assets)"
else
