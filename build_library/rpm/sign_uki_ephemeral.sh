...
# uki_install.sh to sign the embedded IPE policy.
...
if [[ -n "${EPHEMERAL_CERT_DIR}" ]]; then
    # Reuse the cert that already signed the embedded IPE policy. The same cert
    # must sign the UKI and be enrolled in the VM db so the kernel can verify
    # both signatures against .platform.
