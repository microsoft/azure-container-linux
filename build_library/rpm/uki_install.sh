...
OSREL
    ...
    local cmdline=""
    if [[ ${FLAGS_verity} -eq ${FLAGS_TRUE} ]]; then
        # The main UKI remains slot-independent. The active slot addon
        # supplies the data/hash PARTUUIDs, root hash, and credential path.
    ...
    #   slot-b.addon.efi      (PARTUUID for data + hash), root hash,
    #                        root-hash credential reference, and
    #                        acl.slot=<a|b>. Trident swaps this to switch
...
_uki_build_verity_addons() {
    ...
    for slot in a b; do
        ...
        local verity_usr_options="panic-on-corruption"
        if [[ -n "${IPE_VERITY_SIG_PATH}" ]]; then
            verity_usr_options+=",root-hash-signature=${IPE_VERITY_SIG_PATH}"
        fi

        # mount.usr and the IPE policy binding are in the main UKI; the addon
        # carries the slot-specific verity identity and credential reference.
