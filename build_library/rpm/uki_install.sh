...
OSREL
    ...
    # Read filesystem and verity UUIDs from build artifact files.
    # These are random per-image, generated by mkfs.btrfs and veritysetup
    # format respectively, then extracted by disk_util verity.
    local usr_fs_uuid=""
    if [[ -n "${FLAGS_fs_uuid}" && -f "${FLAGS_fs_uuid}" ]]; then
        usr_fs_uuid=$(cat "${FLAGS_fs_uuid}")
        info "UKI/RPM: USR FS UUID = ${usr_fs_uuid}"
    fi

    local verity_superblock_uuid=""
    if [[ -n "${FLAGS_verity_uuid}" && -f "${FLAGS_verity_uuid}" ]]; then
        verity_superblock_uuid=$(cat "${FLAGS_verity_uuid}")
        info "UKI/RPM: Verity superblock UUID = ${verity_superblock_uuid}"
    fi
    ...
    if [[ ${FLAGS_verity} -eq ${FLAGS_TRUE} ]]; then
        ...
        if [[ -z "${usr_fs_uuid}" ]]; then
            die "UKI/RPM: Verity enabled but no FS UUID available"
        fi
        if [[ -z "${verity_superblock_uuid}" ]]; then
            die "UKI/RPM: Verity enabled but no verity UUID available"
        fi
        cmdline="mount.usr=/dev/mapper/usr mount.usrflags=ro"
        cmdline+=" systemd.verity_usr_data=UUID=${usr_fs_uuid}"
        cmdline+=" systemd.verity_usr_hash=UUID=${verity_superblock_uuid}"
        cmdline+=" systemd.verity_usr_options=panic-on-corruption"
        cmdline+=" usrhash=${usr_hash}"
    else
        if [[ -n "${usr_fs_uuid}" ]]; then
            cmdline="mount.usr=UUID=${usr_fs_uuid} mount.usrflags=ro"
        else
            # Fallback: read partition UUID from disk_layout.json
