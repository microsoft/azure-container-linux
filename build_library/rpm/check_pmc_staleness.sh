...
is_older() {
    if command -v rpmdev-vercmp &>/dev/null; then
        local rc=0
        rpmdev-vercmp "$1" "$2" >/dev/null 2>&1 || rc=$?
        [[ ${rc} -eq 12 ]]
    else
...
if [[ -n "${FASTTRACK_REPO_FILE:-}" ]] && [[ -f "${FASTTRACK_REPO_FILE}" ]]; then
    ...
    dnf_output="$(timeout 120 /usr/bin/dnf5 repoquery \
        ...
        ${pkg_list} 2>/dev/null)" || { error "Failed to query FastTrack and PMC repositories"; exit 1; }
else
    ...
    dnf_output="$(timeout 120 /usr/bin/dnf5 repoquery \
        ...
        --available --latest-limit=1 \
        --queryformat="%{name} %{version}-%{release}\n" \
        ${pkg_list} 2>/dev/null)" || { error "Failed to query PMC repositories"; exit 1; }
fi
