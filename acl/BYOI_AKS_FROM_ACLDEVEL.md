...
> **Note:** The acldevel base image must be replicated to `westus3` for the MAP pipeline to succeed. If replication is not done by default, you may need to request it.
...
| Issue | Resolution |
|---|---|
| MAP pipeline fails with image replication error | Ensure the acldevel base image is replicated to `westus3`. Contact the team for PIM approval on the `EdgeOS_Mariner_Platform_AKS_test` subscription if needed. |
