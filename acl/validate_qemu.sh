...
start_vm_qemu_from_build_scripts_arm64() {
    local vm_image_path="$1"

    local boot_script="__build__/images/images/arm64-usr/latest/acl_production_qemu_uefi.sh"

    if [[ ! -f "$boot_script" ]]; then
        error "Boot script not found for arm64: $boot_script"
        error "Expected boot script to be generated by build_vm_images.sh with --build-vm-image option"
        exit 1
    fi

    info "Booting ARM64 VM using build script: $boot_script"
    bash "$boot_script"
}
...
start_vm_qemu() {
    ...
    if [[ "${board}" == "arm64-usr" ]] && is_azure_linux_3; then
        info "For arm64, there is no equivalent package which provides VMF firmware in Azure Linux 3."
        info "Booting a qemu vm with sdk containers provided arm64 VMF vars & fd"
        start_vm_qemu_from_build_scripts_arm64 "${vm_image_path}"
        return
    fi

    if [[ "${board}" == "arm64-usr" ]] && ! command -v qemu-system-aarch64 &>/dev/null; then
        error "qemu-system-aarch64 not found. Install with: sudo apt-get install -y qemu-system-arm qemu-efi-aarch64"
        exit 1
    fi
