...
test_uki_binds_policy_before_writing_cmdline() {
    ...
    local declaration_line append_line write_line addon_function
    ...
    addon_function="$(sed -n '/^_uki_build_verity_addons() {/,/^}/p' "${uki_install}")"
    grep -Fq 'verity_usr_options+=",root-hash-signature=${IPE_VERITY_SIG_PATH}"' <<< "${addon_function}"
    grep -Fq 'cmdline+=" systemd.verity_usr_options=${verity_usr_options}"' <<< "${addon_function}"
    if sed -n '/^uki_provision_rpm() {/,/^_uki_build_firstboot_addon() {/p' "${uki_install}" |
        grep -Fq 'cmdline+=" systemd.verity_usr_options='; then
        echo "slot-specific verity options were embedded in the main UKI" >&2
        return 1
    fi
}
