...
# full GPU sysext test flow. By default, it generates an ACR access token
# using the developer's Azure identity and passes it to the VM so the
# test script can pull sysexts from ACR via ORAS.
#
# Alternatively, use --scp-sysexts to SCP local .raw files to the VM
# instead of pulling from a registry.
#
# Modes:
#   Default (ORAS pull from ACR):
#     - VM must have outbound network access to ${ACR_REGISTRY}
#       (default: acldevel.azurecr.io)
#     - The host running this script must be able to mint an ACR access
#       token (i.e. have an active `az login` session or run with an MSI
#       that has AcrPull on the registry).
...
#     - VM does NOT need any ACR access, but the SSH user must have
...
#                       vgpu      (NV A10)
#   SCRIPT_DIR    — Path to acl-scripts root (set automatically by validate_common.sh)
#
...
source "${SCRIPT_DIR:-$(cd "$(dirname "$0")/.." && pwd)/..}/acl/validate_common.sh"
...
ACR_REGISTRY="${ACR_REGISTRY:-acldevel.azurecr.io}"
ACL_GPU_REPO="${ACL_GPU_REPO:-${ACR_REGISTRY}/azurelinux/3.0/azure-container-linux}"

info "GPU Sysext Host Test"
info "  VM IP:        ${VM_IP}"
info "  SSH User:     ${SSH_USER}"
info "  Driver:       ${GPU_DRIVER_FLAVOR} (${GPU_DRIVER_SYSEXT})"
info "  Mode:         $(if ${SCP_SYSEXTS}; then echo "SCP (local files)"; else echo "ORAS (ACR pull)"; fi)"
...
if ${SCP_SYSEXTS}; then
    ...
    info "Generating ACR access token for ${ACR_REGISTRY}..."
