...
Options:
    ...
    --catalog PATH      Path to package_catalog.sh
...
def parse_package_catalog(catalog_path: Path) -> dict:
    """Parse package_catalog.sh and extract package mappings."""
    catalog = {
        'mappings': {},      # portage_pkg -> rpm_name
    ...
    # Pattern to match catalog entries like:
...
def format_text(results: dict, catalog: dict, installed: dict) -> str:
    ...
    if results['uncataloged']:
        ...
        lines.append("  Consider adding them to package_catalog.sh")
        lines.append("")
...
def format_markdown(results: dict, catalog: dict, installed: dict) -> str:
    ...
    if results['uncataloged']:
        ...
        lines.append("")
        lines.append("```bash")
        lines.append("# Add to package_catalog.sh:")
    ...
    if results['skip_entries']:
        ...
        lines.append("")
        lines.append("<details>")
        lines.append("<summary>Click to expand ({} packages)</summary>".format(len(results['skip_entries'])))
        lines.append("")
...
def main():
    ...
    default_catalog = script_dir.parent / 'build_library' / 'rpm' / 'package_catalog.sh'
    
    parser.add_argument('--build-dir', '-d', type=Path, default=default_build_dir,
                        help=f'Path to build output directory')
    parser.add_argument('--catalog', '-c', type=Path, default=default_catalog,
                        help=f'Path to package_catalog.sh')
