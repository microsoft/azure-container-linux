...
- At boot, `systemd-veritysetup` activates the verity device using slot-specific parameters delivered via a per-slot systemd-stub addon: `systemd.verity_usr_data=PARTUUID=<usr-data-partition>`, `systemd.verity_usr_hash=PARTUUID=<usr-hash-partition>`, `usrhash=<root-hash>`, and `systemd.verity_usr_options=panic-on-corruption,root-hash-signature=/.extra/credentials/verity-usr-<root-hash>.p7s.cred`.
- The main UKI cmdline stays slot-independent (`mount.usr=/dev/mapper/usr`). Trident switches slots by replacing the active slot addon in `<uki>.efi.extra.d/`, so the same signed UKI boots either A or B. The detached root-hash signature and IPE policy are stored beside the UKI as `*.cred` companions, which systemd-stub transports into the initramfs at `/.extra/credentials/`. The secondary GRUB boot path instead builds the verity cmdline from filesystem/verity UUIDs (`UUID=<btrfs-uuid>` / `UUID=<verity-uuid>`).
...
- On IPE-capable (`audit`) images, the signed policy candidate CMS is staged at `${BUILD_DIR}/acl-ipe-policy/acl-ipe-policy.p7b.cred` and installed as a per-UKI `.extra.d` credential companion at `EFI/Linux/<uki>.efi.extra.d/acl-ipe-policy.p7b.cred`. The main UKI cmdline includes an `acl.ipe.policy_sha256=<hash>` token that binds the credential to the signed kernel command line, while each slot addon references the hash-matched `/usr` credential for that slot. On Azure, the initramfs loader validates and loads the policy, activating it only when IMDS requests `ipe=audit` or the legacy alias `ipe=permissive` (runtime enforcement state `0`). Runtime mode is never persisted as image metadata. Development and test builds use `ephemeral` signing mode. Production IPE validation uses `esrp` signing mode, while normal production pipeline defaults remain `disabled` until activation is approved. The optional `${BUILD_DIR}/ipe-signing-mode` marker records signing provenance; its absence means disabled.
