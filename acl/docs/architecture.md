...
- IPE capability is opt-in through `ACL_IPE_CAPABLE=true`; unspecified and unsupported image builds omit the IPE assets. Pipelines enable it only for Azure Secure Boot UKI images.
- On IPE-capable images, the attached signed policy is stored at `/usr/lib/ipe/acl.pol.p7b`, and the initramfs contains the early loader. Development and test builds use the default `ephemeral` policy mode, which signs the source policy with a per-build key. Production builds can select `external` mode and supply an attached DER PKCS#7 artifact signed outside the build; the build verifies the signature and requires its extracted content to exactly match the source policy before sealing `/usr`. The policy remains inactive by default; before `switch_root`, Azure IMDS can request only `ipe=permissive`, which activates the policy with runtime enforcement state `0`.
